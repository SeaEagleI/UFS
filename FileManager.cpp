...
void FileManager::Open() {
    ...
    /* 用户组管理 & 权限控制：
     * 文件主为所欲为；
     * 其他人的Open方法参数必须满足i_mode的要求，否则失败。
     * 注：所有登录用户均在同一用户组*/
//    if(pINode->i_uname==string("")) {
//        u.u_error = User::U_ENOENT;
//        return;
//    }
//    else if(/*pINode->i_uname!=string("") && */pINode->i_uname!=u.username){
//        unsigned int IRWG = (pINode->i_mode & (~INode::MASK))>>4; //possible results: 0b00,0b01,0b10,b11
//        unsigned int IREQ = u.arg[1]>>4;
//        cout<<IRWG<<" "<<IREQ<<endl;
//        if(IRWG%2 < IREQ%2 || (IRWG/2)%2 < (IREQ/2)%2){
...
void FileManager::Creat() {
    ...
    /* 权限控制：只有文件主才能覆盖自己之前创建的文件*/
//    if(pINode->i_uname==string("")) {
//        pINode->i_uname = u.username;
//    }
//    else if(pINode->i_uname!=u.username){
//        u.u_error = User::U_EACCES;
//        return;
//    }
    ...
    /* 修改时间/mode */
    pINode->i_mode = newACCMode | (pINode->i_mode & INode::MASK);
    pINode->i_mtime = (int)Utility::time(NULL);
    pINode->i_atime = (int)Utility::time(NULL);
    pINode->IUpdate((int)Utility::time(NULL));
//    cout<<pINode->i_mode<<endl;
//    cout<<pINode->i_uname<<endl;
//    cout<<pINode->i_mtime<<endl;
//    cout<<pINode->i_atime<<endl;
}
...
INode* FileManager::MakNode(unsigned int mode) {
    ...
    pINode->i_mode = (pINode->i_mode & INode::MASK) | mode | INode::IALLOC;
    pINode->i_nlink = 1;
...
void FileManager::UnLink() {
    ...
    if (NULL == pDeleteINode || (pDeleteINode->i_mode & INode::IFMT) == INode::IFDIR) {
        cout<<"非文件"<<endl;
        return;
    }
...
void FileManager::Status(){
    INode* fINode;
    User& u = g_User;

    fINode = this->NameI(FileManager::OPEN);

    int link = 2;
    string file_class = "文件夹";
    if ((fINode->i_mode & INode::IFMT) != INode::IFDIR) {
        file_class = "普通文件";
        link = 1;
    }

    cout<<"文件: "<<u.dirp <<"\t"<<"文件类型："<<file_class<<"\n";

    char size [150], atime[50], mtime[50];
    sprintf(size, "大小： %d\t设备号: %d\nInode: %d\t硬链接数: %d\t文件打开数: %d\n", fINode->i_size, fINode->i_dev, fINode->i_number, link, fINode->i_opencount);
    cout<<size;
//    sprintf(atime, "%d\t", );
    Utility::stamp_to_standard((time_t)fINode->i_atime, atime);
    Utility::stamp_to_standard((time_t)fINode->i_mtime, mtime);
//    sprintf(mtime, "%d\t", fINode->i_mtime);
    cout<<"最后访问时间: "<<atime<<"\n";
    cout<<"最后修改时间: "<<mtime<<"\n";
}

void FileManager::Rmdir() {
    //注意删除文件夹有磁盘泄露
    INode* pINode;
    INode* pDeleteINode;
    User& u = g_User;
...
void FileManager::Ls() {
    ...
    u.ls = Utility::ls_headers + Utility::ls_seperators;
    while (u.IOParam.count) {
        ...
        if (0 == u.dent.m_ino)
            continue;
        fINode = this->inodeTable->IGet(u.dent.m_ino);
        string file_class = "d";
        if ((fINode->i_mode & INode::IFMT) != INode::IFDIR) file_class="-";
        file_class+="\t";
        string space = "\t";
//        string rwx = "rwxr-x--x\t";
        string rwx = Utility::mode_to_str(fINode->i_mode)+string("\t");
        char size [50], time[50];
        sprintf(size, "%d\t", fINode->i_size);
        Utility::stamp_to_standard((time_t)fINode->i_mtime, time);
        u.ls += file_class;
        u.ls += rwx;
        u.ls += (u.dent.name + space);
//        u.ls += (fINode->i_uname+space);
        u.ls += size;
        u.ls += time;
        u.ls += "\n";
    }
...
void FileManager::Ls2() {
    ...
    pINode = this->NameI(FileManager::OPEN);
    if (NULL == pINode || (pINode->i_mode & INode::IFMT) != INode::IFDIR) {
        cout<<"非文件夹!"<<endl;
        return;
    }
...
/* 显示历史信息 */
void FileManager::History(){
    User &u = g_User;
//    char data[500];
    ifstream infile;
    infile.open("./history", ios::in);
    if (!infile.good()){
        cout<<"文件打开失败"<<endl;
        return;
    }
